...
@interface SecondViewController ()<MFCyclePagerViewDelegate>
...
- (__kindof UICollectionViewCell *)pagerView:(MFCyclePagerView *)pagerView cellForItemAtIndex:(NSInteger)index{
    if (index%2 == 0) {
        ...
        if (numObj) {
            cell.itemLabel.text = [NSString stringWithFormat:@"默认类型1的第%zd列",index];
        }else{
    ...
    cell.imgView.image = nil;
    if (numObj) {
        cell.itemLabel.text = [NSString stringWithFormat:@"默认类型2的第%zd列",index];
    }else{
        cell.itemLabel.text = nil;
    }
    return cell;
...
- (MFCyclePagerView *)bannerView{
    if (!_bannerView) {
        _bannerView = [[MFCyclePagerView alloc] initWithFrame:self.view.bounds];
        ...
        _bannerView.scrollDirection = MFCycleScrollDirectionVertical;
